(function(window){
    ...
    Tree.prototype = {
        ...
        initBloom: function() {
            ...
            var r = bloom.radius || (isPortrait ? height * 0.4 : 240);
            for (var i = 0; i < num; i++) {
        ...
        // 创建樱花
        createSakura: function(width, height) {
            var x = random(0, width);
            var y = -20; // 从屏幕上方开始
            return new Sakura(this, new Point(x, y), random(0.6, 1), random(0, 360), random(0.5, 1), null, random(10, 20));
        },
        ...
        jump: function() {
            ...
            if ((blooms.length && blooms.length < 3) || !blooms.length) {
                ...
                // 根据屏幕比例调整参数
                var isPortrait = width < height;
                var r = isPortrait ? height * 0.4 : 240;
                for (var i = 0; i < random(1,2); i++) {
                    blooms.push(this.createBloom(width / 2 + width, height, r, figure, null, 1, null, 1, new Point(random(-100, width), height + 50), random(200,300)));
                }
            }
        }
    }
    ...
    // 樱花类
    Sakura = function(tree, point, alpha, angle, scale, place, speed) {
        this.tree = tree;
        this.point = point;
        // 樱花颜色：粉色系
        this.color = 'rgb(' + (255 - random(0, 30)) + ',' + (180 - random(0, 50)) + ',' + (200 - random(0, 50)) + ')';
        this.alpha = alpha || random(0.6, 1);
        this.angle = angle || random(0, 360);
        this.scale = scale || 0.1;
        this.place = place;
        this.speed = speed;
        this.alphaDecay = 0.005; // 透明度衰减速度
    }
    Sakura.prototype = {
        draw: function() {
        ...
        jump: function() {
            ...
            if (s.point.x < -20 || s.point.y > height + 20 || s.alpha <= 0) {
                s.tree.removeBloom(s);
            } else {
                s.draw();
                // 飘落效果
                s.point.x += Math.sin(s.angle) * 2;
                s.point.y += 3;
                s.angle += 0.05;
                // 透明度渐变
